...
export default class Home extends React.Component<HomeProps, HomeState> {
  ...
  public render(): JSX.Element {
    ...
    let screenings = this.props.screenings?.filter(s => s.recommendScreening);

    return (
      <div className="home-view">
        {(patient === undefined) ? '' :
            <p><b>{patient?.fullName}</b> ({patient?.gender}) Age {patient?.age}</p>
        }
        ...
        : <div>
            ...
            {(screenings !== undefined && screenings.length === 0)
                ? <p>You are up to date on cancer screenings.</p>
                : <ul>
